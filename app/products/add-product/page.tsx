...
interface ProductData {
  ...
  an_hien: boolean; // Thêm an_hien
  hot: boolean; // Thêm hot
}

// Interface cho dữ liệu variant
interface VariantData {
  // sku: string; // SKU will be generated by API, so removed from frontend input
  kich_thuoc: string;
  mau_sac: string;
  gia: number;
  gia_km: number | null; // Thêm gia_km
  so_luong: number;
  hinh_chinh: string; // URL của ảnh chính (sau khi upload)
  hinh_thumbnail: string[]; // URLs của ảnh thumbnail (sau khi upload)
  mainImageFile?: File | null; // File tạm thời cho ảnh chính
  thumbnailFiles?: File[]; // Files tạm thời cho ảnh thumbnail
}
...
export default function FormAddProduct() {
  ...
  const [productData, setProductData] = useState<ProductData>({
    ...
    meta_description: '',
    meta_keywords: '',
    an_hien: true, // Mặc định hiển thị
    hot: false, // Mặc định không phải sản phẩm hot
  });

  // Khởi tạo với một biến thể rỗng
  const [variants, setVariants] = useState<VariantData[]>([
    {
      // sku: '', // Removed SKU from initial state
      kich_thuoc: '',
      mau_sac: '',
      gia: 0,
      gia_km: null, // Mặc định là null
      so_luong: 0,
      hinh_chinh: '',
      hinh_thumbnail: [],
      mainImageFile: null,
  ...
  const [loading, setLoading] = useState(true); // Để kiểm soát trạng thái tải dữ liệu ban đầu
  const [saving, setSaving] = useState(false); // Để kiểm soát trạng thái lưu form
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError(err.message || "Không thể tải danh mục hoặc thương hiệu. Vui lòng thử lại.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleAddVariant = () => {
    setVariants((prev) => [
      ...prev,
      {
        // sku: "", // Removed SKU from new variant
        kich_thuoc: "",
        mau_sac: "",
        gia: 0,
        gia_km: null,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate all variants have main images
      for (const v of variants) {
        if (!v.mainImageFile && !v.hinh_chinh) { // hinh_chinh here is for existing images (not applicable for add product, but good for robustness)
      ...
      const variantsToSubmit = await Promise.all(
        variants.map(async (v) => {
          ...
          if (v.thumbnailFiles && v.thumbnailFiles.length > 0) {
            currentThumbnailUrls = await Promise.all(
              v.thumbnailFiles.map((file) => uploadImageToCloudinary(file, 'product_thumbnails'))
            );
          }

          return {
            // sku: v.sku, // Removed SKU from data sent to API
      ...
      setProductData({ // Reset form after successful submission
        ten_sp: '',
        id_loai: 0,
        id_thuong_hieu: 0,
        mo_ta: '',
      ...
      setVariants([ // Reset variants
        {
          // sku: '', // Removed SKU from reset state
          kich_thuoc: '',
          mau_sac: '',
          gia: 0,
          gia_km: null,
  ...
  if (loading) return (
    <BoCucAdmin title="Đang tải dữ liệu...">
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-gray-600">Đang tải danh mục và thương hiệu...</p>
      </div>
    </BoCucAdmin>
  );

  return (
    <BoCucAdmin title="Thêm Sản Phẩm Mới">
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
        <div className="max-w-6xl mx-auto">
          {/* Header */}
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">Thêm Sản Phẩm Mới</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8 border border-gray-200">
              ...
              {variants.map((variant, index) => (
                <div key={index} className="border border-gray-300 rounded-xl p-6 mb-8 relative bg-gray-50">
                  {variants.length > 1 && (
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div>
                      ...
                      {variant.mainImageFile && (
                        <div className="mt-4">
                          <img
                            src={URL.createObjectURL(variant.mainImageFile)}
                    ...
                    <div>
                      ...
                      {variant.thumbnailFiles && variant.thumbnailFiles.length > 0 && (
                        <div className="mt-4">
                          <div className="grid grid-cols-3 gap-3">
                            {variant.thumbnailFiles.map((file, idx) => (
                          ...
                          <p className="text-sm text-gray-600 mt-3 text-center">
                            Đã chọn {variant.thumbnailFiles.length} ảnh thumbnail
                          </p>
                        </div>
                      )}
                    </div>
